...
locals {
  prefix = "p4-prod"
  tags   = { environment = "prod", project = "helix-core-on-azure", owner = "chris" }
...
# ---- cross-region standby (prod only) -------------------------------------
# This is the module that makes the repo a DR asset rather than a deployment
# asset. It is also what a failover drill exercises.

resource "azurerm_resource_group" "dr" {
  count    = var.enable_standby ? 1 : 0
  name     = "${local.prefix}-dr-rg"
...
module "standby" {
  ...
  count                = var.enable_standby ? 1 : 0
  name                 = "${local.prefix}-${local.dr_loc}-standby-01"
  location             = var.dr_location
  resource_group_name  = azurerm_resource_group.dr[0].name
